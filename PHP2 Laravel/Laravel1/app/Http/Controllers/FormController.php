...
class FormController extends BaseController
{
    ...
    public function validarFormulario()
    {
        ...
        $utiles->filtrar('estado', "", $errores);
        $utiles->filtrar('email', "", $errores);
        ...
        if (!is_dir('uploads/fotos_trabajo')) {
            mkdir('uploads/fotos_trabajo', 0777, true);

            // Manejar subida del fichero resumen (PDF)
            if ($utiles->filtrarPDF('fichero_resumen', $errores)) {
                $ficheroResumen = basename($_FILES['fichero_resumen']['name']);
                $destinoResumen = 'uploads/resumenes/' . $ficheroResumen;
                if (!move_uploaded_file($_FILES['fichero_resumen']['tmp_name'], $destinoResumen)) {
                    $errores['fichero_resumen'] = 'Error al subir el fichero resumen';
                }
            }

            // Manejar subida de fotos
            $fotosTrabajos = [];
            if (isset($_FILES['fotos_trabajo'])) {
                foreach ($_FILES['fotos_trabajo']['tmp_name'] as $key => $tmpName) {
                    $nombreFoto = basename($_FILES['fotos_trabajo']['name'][$key]);
                    $destinoFoto = 'uploads/fotos_trabajo/' . $nombreFoto;
                    if (move_uploaded_file($tmpName, $destinoFoto)) {
                        $fotosTrabajos[] = $destinoFoto;
                    } else {
                        $errores['fotos_trabajo'][] = 'Error al subir la foto ' . $nombreFoto;
                    }
                }
            }
